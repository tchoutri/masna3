...
pub type Msg {
  ...
  UserSubmittedForm
  ApiReturnedCompletedProcess(Result(response.Response(String), rsvp.Error))
}
...
pub type Model {
  Model(
    ...
    complete_process_response: Option(
...
pub fn send(process_id: ProcessId) -> Effect(Msg) {
  ...
  let url = config.api_base_url <> "/processes/" <> id <> "/complete"
  let handler = rsvp.expect_ok_response(ApiReturnedCompletedProcess)
...
pub fn init() -> Model {
  ...
  Model(process_id:, complete_process_response: None, validation_errors: [])
}
...
pub fn update(model: Model, msg: Msg) -> #(Model, Effect(Msg)) {
  case msg {
    ...
    UserSubmittedForm -> {
      ...
      let new_model =
        Model(..model, validation_errors:, complete_process_response: None)
      case validation_errors {
        [] -> #(new_model, send(new_model.process_id))
        _ -> #(new_model, effect.none())
      }
    }
    ApiReturnedCompletedProcess(Ok(completed_process)) -> #(
      Model(..model, complete_process_response: Some(Ok(completed_process))),
      effect.none(),
    )
    ApiReturnedCompletedProcess(Error(err)) -> #(
      Model(..model, complete_process_response: Some(Error(err))),
      effect.none(),
    )
  }
}
